...
ASSISTANT_ID_SCENARISTE_MDD  = st.secrets["ASSISTANT_ID_SCENARISTE_MDD "]
...
if "thread_ID_SCENARISTE_MDD " not in st.session_state:
    st.session_state.thread_ID_SCENARISTE_MDD  = None
...
def initialize_thread(assistant_role):
    if assistant_role == "scenariste":
        if st.session_state.thread_ID_SCENARISTE_MDD  is None:
            thread = client.beta.threads.create()
            st.session_state.thread_ID_SCENARISTE_MDD  = thread.id
...
def send_message_and_stream(assistant_id, assistant_role, user_input):
    ...
    thread_id = st.session_state.thread_ID_SCENARISTE_MDD  if assistant_role == "scenariste" else st.session_state.thread_id_ecrivain
...
def start_story():
    ...
    user_input = "Commence par le checkpoint 2."
    scenariste_plan = send_message_and_stream(ASSISTANT_ID_SCENARISTE_MDD _SEA, "scenariste", user_input)
...
def generate_plan_and_pass_to_writer(user_input):
    ...
    scenariste_plan = send_message_and_stream(ASSISTANT_ID_SCENARISTE_MDD _SEA, "scenariste", scenariste_prompt)
    # Après avoir récupéré le plan, envoyer ce plan à l'écrivain
